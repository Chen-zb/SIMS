...
from datasets.cityscape.create_dataset import CityScape
...
from utils import ConfMatrix, create_optimizer_scheduler, depth_error, model_fit


def parser_args():
    parser = argparse.ArgumentParser(description="SIMS for cityscapes")
    ...
    parser.add_argument("--model_cfg", default="configs/sam2.1/sam2.1_hiera_l_city.yaml")
...
def build_predictor(params, device, local_rank):
    ...
    domain2task2slice = {"CityScapes": {"seg": slice(0, 7), "depth": slice(7, 8)}}
...
def collect_predictions(predictor, img, org_dataset):
    ...
    return seg_pred, dep_pred


def forward_and_losses(predictor, img, seg_gt, dep_gt, org_dataset):
    seg_pred, dep_pred = collect_predictions(predictor, img, org_dataset)
    seg_pred = F.log_softmax(seg_pred, dim=1)
    loss_seg = model_fit(seg_pred, seg_gt, "semantic")
    loss_dep = model_fit(dep_pred, dep_gt, "depth")
    return [loss_seg, loss_dep]
...
def evaluate(predictor, test_loader, device):
    with torch.no_grad():
        ...
        conf_mat_city = ConfMatrix(7)
        avg_loss_seg = 0
        avg_loss_dep = 0
        avg_cost_dep = [0, 0]

        for _ in range(val_batch):
            val_img, val_seg_gt, val_dep_gt = next(val_dataset)
            val_img, val_dep_gt = val_img.to(device), val_dep_gt.to(device)
            val_seg_gt = val_seg_gt.long().to(device)

            seg_pred, dep_pred = collect_predictions(
                predictor.module,
                val_img,
                "CityScapes",
            )
            ...
            conf_mat_city.update(seg_pred.argmax(1).flatten(), val_seg_gt.flatten())
        ...
        avg_cost_seg = [0, 0]
        avg_cost_seg[0], avg_cost_seg[1] = conf_mat_city.get_metrics()

    print(
        "CITY | Test: {:.4f} {:.4f} {:.4f} | {:.4f} {:.4f} {:.4f}".format(
            avg_loss_seg,
            avg_cost_seg[0],
            avg_cost_seg[1],
            avg_loss_dep,
...
def main():
    ...
    for epoch in range(total_epoch):
        ...
        for batch_index in range(train_batch):
            ...
            img, seg_gt, dep_gt = next(train_dataset)
            img, dep_gt = img.to(device), dep_gt.to(device)
            seg_gt = seg_gt.long().to(device)
            batch = (img, seg_gt, dep_gt, "CityScapes")
        ...
        if dist.get_rank() == 0:
            ...
            if epoch % 10 == 0:
                ...
                torch.save(current_state_dict, params.output_folder + "/" + str(epoch) + ".pth")
