...
class SV_Fair(WeightMethod):
    ...
    def get_weighted_loss(
        ...
        g, GTG, w_cpu = self.svfair(grads, alpha=self.alpha)
        self.overwrite_grad(shared_parameters, g, grad_dims)
        return GTG, w_cpu

    def svfair(self, grads, alpha=1.0):
        GG = grads.t().mm(grads).cpu()

        weights = compute_sharply_value(grads, self.n_tasks, alpha=alpha)

        x_start = np.ones(self.n_tasks) / self.n_tasks
        A = GG.data.cpu().numpy()

        def objfn(x):
            return ((np.dot(A, x)) ** 2) - (weights / x)
...
def compute_sharply_value1(grads, task_num, alpha=1.0):
    def compute_cosine_similarity_blockwise_cpu(G, block_size=10 ** 6):
        G = G.to('cuda')
        G = G.T
        norms = torch.norm(G, p=2, dim=1, keepdim=True)
        G_normalized = G / norms
        num_rows = G.shape[0]
        L = torch.zeros((num_rows, num_rows), device='cuda')

        for i in range(0, G.shape[1], block_size):
            block = G_normalized[:, i:i + block_size]
            L += torch.mm(block, block.T)

        return L

    # # This Function is used for VolDet. You can choose either VolDet or VolDetPro
    # def value_of_subset(K_mat, subset):
    #     if len(subset) == 0:
    #         return 1.0
    #     idx = list(subset)
    #
    #     subK = K_mat[np.ix_(idx, idx)]
    #     return np.linalg.det(subK)

    # This Function is used for VolDetPro. You can choose either VolDet or VolDetPro
    def value_of_subset(K_mat, subset):
        if len(subset) == 0:
            return 1.0
        idx = list(subset)
        # subK = K_mat.detach().cpu().numpy()[np.ix_(idx, idx)]
        subK = K_mat[np.ix_(idx, idx)]
        vol_det = np.linalg.det(subK)
        triu_vals = np.triu(subK, k=1)
        neg_mask = (triu_vals < 0)
        neg_vals = triu_vals[neg_mask]
        NegCosNum = int(neg_vals.size)
        sum_neg = float(neg_vals.sum()) if NegCosNum > 0 else 0.0
        N = task_num
        # VolDetPlus = VolDet + ((sqrt(NegCosNum)+N) * |sum(NegCos)|) / N
        vol_det_plus = vol_det + ((np.sqrt(NegCosNum) + N) * abs(sum_neg)) / N
    ...
    L = compute_cosine_similarity_blockwise_cpu(G, block_size=5 * 10 ** 6).detach().cpu().numpy()

    for i in tasks:
        other_tasks = [t for t in tasks if t != i]
        for r in range(len(other_tasks) + 1):
            for S in itertools.combinations(other_tasks, r):
                S = set(S)
                vS = value_of_subset(L, S)
                vSi = value_of_subset(L, S.union({i}))
                # weight = |S|! * (|N|-|S|-1)! / |N|!
                size_S = len(S)
                weight = (factorial(size_S) * factorial(N - size_S - 1)) / factorial_6

                shapley_values[i] += weight * (vSi - vS)

    shapley_values = shapley_values / alpha
    shapley_values_tensor = torch.tensor(shapley_values, dtype=torch.float32)
    shapley_values_softmax = F.softmax(shapley_values_tensor, dim=0)
    shapley_values_softmax = shapley_values_softmax.cpu().detach().numpy()
    return shapley_values_softmax


def compute_sharply_value(grads, task_num, alpha=1.0, num_samples=1000, block_size=10 ** 6):
    def compute_cosine_similarity_blockwise_gpu(G, block_size=block_size):
        G = G.T
        norms = torch.norm(G, p=2, dim=1, keepdim=True)
        G_normalized = G / norms
        num_rows = G.shape[0]
    ...
    def value_of_subset(K_mat, subset):
        ...
        if subK.ndim == 1:
            return subK[0]
        return cp.linalg.det(subK)

    N = task_num
    shapley_values = np.zeros(N)
    G = torch.clone(grads).to('cuda')
    L = compute_cosine_similarity_blockwise_gpu(G, block_size=5 * 10 ** 6).detach().cpu().numpy()
    ...
    for sample in range(num_samples):
        ...
        for i in permutation:
            new_subset = S.union({i})
            v_S_i = value_of_subset(L, new_subset)
            marginal = v_S_i - v_S
            shapley_values[i] += marginal
            S.add(i)
            v_S = v_S_i
    shapley_values /= num_samples
    shapley_values /= alpha
    shapley_values_tensor = torch.tensor(shapley_values, dtype=torch.float32, device='cuda')
    shapley_values_softmax = F.softmax(shapley_values_tensor, dim=0)
    shapley_values_softmax = shapley_values_softmax.cpu().detach().numpy()
