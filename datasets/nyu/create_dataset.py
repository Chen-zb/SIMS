...
class RandomScaleCrop(object):
    ...
    def __call__(self, img, label, depth, normal, edge=None):
        ...
        if edge is not None:
            edge_ = F.interpolate(depth[None, :, i:i + h, j:j + w], size=(height, width), mode='nearest').squeeze(0)
            return img_, label_, depth_ / sc, normal_, edge_ 
...
#         self.data_len = len(fnmatch.filter(os.listdir(self.data_path + '/image'), '*.npy'))
    ...
    def __getitem__(self, i):
        ...
        if self.mode == 'trainval':
            edge = np.load(self.root + '/edge/' + 'train_{:d}.npy'.format(index))
        else:
            edge = np.load(self.root + '/edge/' + 'val_{:d}.npy'.format(index))
        edge = np.array(edge, dtype=np.float32, copy=False) / 255.
        if edge.shape[-2:] != image.shape[-2:]:
            edge = torch.from_numpy(cv2.resize(edge, image.shape[-2:][::-1], interpolation=cv2.INTER_NEAREST)).unsqueeze(0)

        # apply data augmentation if required
        if self.augmentation:
            image, semantic, depth, normal, edge = RandomScaleCrop()(image, semantic, depth, normal, edge)
            if torch.rand(1) < 0.5:
                image = torch.flip(image, dims=[2])
                semantic = torch.flip(semantic, dims=[1])
                depth = torch.flip(depth, dims=[2])
        ...
        if self.task == 'multi-task':
            if 'train' in self.mode:
                return image.float(), semantic.float(), depth.float(), normal.float(), edge.float()
            else:
                return image.float(), semantic.float(), depth.float(), normal.float(), edge.float(), i
        if self.task == 'semantic':
            return image.float(), semantic.float()
        if self.task == 'depth':
            return image.float(), depth.float()
