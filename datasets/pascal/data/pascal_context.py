...
from datasets.pascal.data import custom_transforms as tr
...
class PASCALContext(data.Dataset):
    ...
    def __init__(self,
        ...
        if do_edge:
            FLAGVALS['edge'] =  cv2.INTER_NEAREST
        if do_semseg:
            FLAGVALS['semseg'] =  cv2.INTER_NEAREST 
        if do_human_parts:
            FLAGVALS['human_parts'] =  cv2.INTER_NEAREST 
        if do_sal:
            FLAGVALS['sal'] =  cv2.INTER_NEAREST
        if do_normals:
            FLAGVALS['normals'] =  cv2.INTER_CUBIC 
        ...
        self.transform = []
        if self.aug:
            self.transform.extend([tr.RandomHorizontalFlip(), tr.ScaleNRotate(rots=(-20, 20), scales=(.75, 1.25),
                                  flagvals=FLAGVALS)])

        self.transform.extend([tr.FixedResize(resolutions={x: (512, 512) for x in FLAGVALS.keys()},
                                             flagvals=FLAGVALS)])
        self.transform.extend([tr.AddIgnoreRegions(), tr.ToTensor(),
