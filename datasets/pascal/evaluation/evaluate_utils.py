...
def get_single_task_meter(database, task):
    ...
    if task == 'semseg':
        from datasets.pascal.evaluation.eval_semseg import SemsegMeter
        return SemsegMeter(database)

    elif task == 'human_parts':
        from datasets.pascal.evaluation.eval_human_parts import HumanPartsMeter
        return HumanPartsMeter(database)

    elif task == 'normals':
        from datasets.pascal.evaluation.eval_normals import NormalsMeter
        return NormalsMeter()

    elif task == 'sal':
        from datasets.pascal.evaluation.eval_sal import SaliencyMeter
        return SaliencyMeter()

    elif task == 'depth':
        from datasets.pascal.evaluation.eval_depth import DepthMeter
        return DepthMeter()

    elif task == 'edge': # Single task performance meter uses the loss (True evaluation is based on seism evaluation)
        from datasets.pascal.evaluation.eval_edge import EdgeMeter
...
def eval_all_results(p):
    ...
    if 'edge' in p.TASKS.NAMES: 
        from datasets.pascal.evaluation.eval_edge import eval_edge_predictions
        results['edge'] = eval_edge_predictions(p, database=p['val_db_name'],
                             save_dir=save_dir)
    
    if 'semseg' in p.TASKS.NAMES:
        from datasets.pascal.evaluation.eval_semseg import eval_semseg_predictions
    ...
    if 'human_parts' in p.TASKS.NAMES: 
        from datasets.pascal.evaluation.eval_human_parts import eval_human_parts_predictions
        results['human_parts'] = eval_human_parts_predictions(database=p['val_db_name'],
                                   save_dir=save_dir, overfit=p.overfit)

    if 'normals' in p.TASKS.NAMES:
        from datasets.pascal.evaluation.eval_normals import eval_normals_predictions
        results['normals'] = eval_normals_predictions(database=p['val_db_name'],
                               save_dir=save_dir, overfit=p.overfit)

    if 'sal' in p.TASKS.NAMES:
        from datasets.pascal.evaluation.eval_sal import eval_sal_predictions
        results['sal'] = eval_sal_predictions(database=p['val_db_name'],
                           save_dir=save_dir, overfit=p.overfit)

    if 'depth' in p.TASKS.NAMES:
        from datasets.pascal.evaluation.eval_depth import eval_depth_predictions
