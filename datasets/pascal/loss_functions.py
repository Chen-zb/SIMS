...
def get_loss(task):
    ...
    if task == 'edge':
        ...
        criterion = CrossEntropyLoss(balanced=True) 
...
class SoftMaxwithLoss(Module):
    ...
    def forward(self, out, label):
        ...
        valid = (label != 255)
        if valid.sum().item() == 0:
            # 全部 ignore：避免 NLLLoss(mean over empty) -> NaN
            return out.sum() * 0.0
...
class BalancedCrossEntropyLoss(Module):
    ...
    def forward(self, output, label, void_pixels=None):
        ...
        # 先构造 valid mask：void_pixels>=0.5 视为无效
        if void_pixels is not None:
            valid = torch.le(void_pixels, 0.5)
        else:
            valid = torch.ones_like(labels, dtype=torch.bool)

        n_valid = valid.sum().item()
        if n_valid == 0:
            # 全无有效像素：避免除 0 / inf / NaN
            return output.sum() * 0.0

        # 只在有效像素上统计 pos/neg
        labels_v = labels[valid]
        num_pos = labels_v.sum()
        num_neg = (1.0 - labels_v).sum()
        num_total = num_pos + num_neg  # == n_valid

        # 权重 w
        if self.pos_weight is None:
            # 防止极端情况下 num_total 为 0（理论上这里不会，因为 n_valid>0）
            w = num_neg / torch.clamp(num_total, min=1.0)
        else:
            w = torch.tensor(float(self.pos_weight), device=output.device, dtype=output.dtype)

        # 原始稳定形式的 logits BCE（保留你代码）
        out_v = output[valid]
        output_gt_zero = torch.ge(out_v, 0).float()
        loss_val = out_v * (labels_v - output_gt_zero) - torch.log(
            1 + torch.exp(out_v - 2 * out_v * output_gt_zero)
        )

        loss_pos = (-labels_v * loss_val).sum()
        loss_neg = (-(1.0 - labels_v) * loss_val).sum()

        final_loss = w * loss_pos + (1 - w) * loss_neg

        if self.size_average:
            final_loss = final_loss / float(n_valid)  # 只除有效像素数
        elif self.batch_average:
            final_loss = final_loss / label.size(0)
...
class DepthLoss(nn.Module):
    ...
    def forward(self, out, label):
        mask = (label != 255)
        if mask.sum().item() == 0:
            # masked_select 为空 -> L1Loss(mean) 为 NaN
            return out.sum() * 0.0
